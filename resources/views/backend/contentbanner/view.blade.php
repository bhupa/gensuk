...
@section('title','Content-Banner-Lists')
@section('content')
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('content-banners.create')}}"> Add Contnent Banner</a></li>
                ...
                <div class="single-table">
                    <div class="table-responsive">
                        <table class="table text-center" id="table">
                            ...
                            @foreach($contentbanners as $key=>$item)
                                <tr id="item-{{$item->id}}">
                                    <td>{{$key+1}}</td>
                                    <td>{{$item->title}}</td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{route('content-banners.edit', $item->slug)}}" class="edit-modal btn btn-info btn-circle btn-sm"
                                           data-info="">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- @endif --}}
                                        {{-- @if(auth()->user()->can('delete-banner')) --}}
                                        <a href="javascript:void(0)" class="delete-content-banner btn btn-danger btn-circle btn-sm"
                                           data-type="{{$item->id}}">
...
@section('js_script')
            <script>
                $(document).ready( function () {
                    ...
                    $('#table').on('click','.delete-content-banner',function(event){
                        event.preventDefault();
                        $object = $(this);
                        var id  = $(this).attr('data-type');
                        var url = baseUrl+"/content-banners/"+id;
                        swal({
                            title: 'Are you sure?',
                    ...
                    $("#table").on("click", "#change-status", function () {
                        ...
                        swal({
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ route('content-banners.change-status') }}",
                                    data: {
                                        'id': id,
                                    },
                                    headers: {
                                    ...
                                    success: function (response) {
                                        ...
                                        if (response.contentbanner.is_active == 1) {
