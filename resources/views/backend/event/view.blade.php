                        ...
                        <table class="table text-center" id="table">
                            ...
                            @foreach($events as $key=>$item)
                                <tr id="item-{{$item->id}}">
                                    ...
                                    <td>
                                        @if($item->volunters == '1')
                                            <a href="javascript:void(0)"  data-type="{{$item->id}}" id="volunters" class="edit-modal btn btn-sm btn-circle btn-success published"  title="change-status"
                                               data-toggle="tooltip"> <i class="fas fa-check" aria-hidden="true"></i></a>
                                        @else
                                            <a href="javascript:void(0)"  data-type="{{$item->id}}" id="volunters" class="edit-modal btn btn-sm btn-circle btn-success unpublished"   title="change-status"
                                    ...
                                    <td>
                                        ...
                                        <a href="{{route('events.edit', $item->slug)}}" class="edit-modal btn btn-info btn-circle btn-sm"
                                           data-info="">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{-- @endif --}}
                                        {{-- @if(auth()->user()->can('delete-banner')) --}}
                                        <a href="javascript:void(0)" class="delete-events btn btn-danger btn-circle btn-sm"
                                           data-type="{{$item->id}}">
...
@section('js_script')
            <script>
                $(document).ready( function () {
                    ...
                    $('#table').on('click','.delete-events',function(event){
                    ...
                    $("#table").on("click", "#change-status", function () {
                        ...
                        swal({
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ route('events.change-status') }}",
                                    data: {
                                        'id': id,
                                    },
                                    headers: {
                    ...
                    $("#table").on("click", "#volunters", function () {
                        $object = $(this);
                        var id = $(this).attr('data-type');
                        swal({
                            title: 'Are you sure?',
                            text: 'Do you want to Add or Remove Volunters',
                            type: 'warning',
                            showCancelButton: true,
                            confirmButtonText: 'Yes, change it!',
                            cancelButtonText: 'No, keep it'
